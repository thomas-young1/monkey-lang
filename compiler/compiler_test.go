...
func TestGlobalLetStatemnts(t *testing.T) {
	tests := []compilerTestCase{
		{
			input: `
			let one = 1;
...
func TestCompilerScopes(t *testing.T) {
	...
	currentInstructions := compiler.scopes[compiler.scopeIndex].instructions
	if len(currentInstructions) != 1 {
		t.Errorf("instructions length wrong. expected: %d, got: %d", 1,
	...
	if compiler.symbolTable.Outer != nil {
		t.Errorf("compiler modified glboal symbol table incorrectly")
	}
	...
	if len(currentInstructions) != 2 {
		t.Errorf("instructions length wrong. expected: %d, got: %d", 2,
			len(compiler.scopes[compiler.scopeIndex].instructions))
	}
...
func testStringObject(expected string, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("object has wrong value. expected: %q, got: %s", expected, result.Value)
	}
